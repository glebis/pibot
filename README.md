...
- **Probe cleanup**: every message the harness sends — and the bot's direct reply to it — is deleted via telethon ~10s after its scenario completes, so live-test traffic never litters real chats (`--keep-probes` disables).
